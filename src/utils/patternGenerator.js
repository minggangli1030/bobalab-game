...
export const patternGenerator = {
  ...
  generateTypingPattern(level) {
    const patterns = {
      // Easy patterns (1-5)
      easy: [
        'hello world', 'quick test', 'type this', 'easy mode', 'simple text',
        'good job', 'keep going', 'nice work', 'almost done', 'level up'
      ],
      // Medium patterns (6-10) - mixed case
      medium: [
        'HeLLo WoRLd', 'QuIcK tEsT', 'TyPe ThIs', 'MeDiUm MoDe', 'MiXeD cAsE',
        'KeEp GoInG', 'NiCe WoRk', 'AlMoSt DoNe', 'LeVeL uP', 'ChAlLeNgE'
      ],
      // Hard patterns (11-15) - symbols and numbers
      hard: [
        'Test@123', 'Go4It!Now', 'X9%Y8&Z7*', 'P6!Q5?R4+', 'Z1@Y2#X3$',
        'M7&N8*O9!', 'J4%K5^L6&', 'D1!E2@F3#', 'A7$B8%C9^', 'V3*W4+X5='
      ]
    };
    ...
    return {
      ...
      difficulty: level <= 5 ? 'easy' : level <= 10 ? 'medium' : 'hard',
      difficultyLabel: level <= 5 ? 'Easy' : level <= 10 ? 'Medium' : 'Hard',
      complexity: level <= 5 ? 'letters_only' : level <= 10 ? 'mixed_case' : 'symbols_numbers'
    };
  },
