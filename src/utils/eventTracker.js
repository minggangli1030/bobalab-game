...
export const eventTracker = {
  ...
  async trackTaskAttempt(taskId, attempt, isCorrect, timeTaken, answer, expected) {
    ...
    return this.logEvent('task_attempt', {
      ...
      averageTime: attemptHistory.reduce((sum, a) => sum + a.timeTaken, 0) / attemptHistory.length,
  ...
  async trackTaskComplete(taskId, attempts, totalTime, finalAccuracy) {
    ...
    const performanceMetrics = this.calculatePerformanceMetrics(taskId, attemptHistory, totalTime);
    ...
    return this.logEvent('task_complete', {
      ...
      averageAccuracy: attemptHistory.reduce((sum, a) => sum + a.accuracy, 0) / attemptHistory.length,
      ...
      // Additional insights
      insights: {
        struggledWith: this.identifyStrugglePoints(attemptHistory, mistakesByType),
  ...
  async trackGameComplete(totalTime, totalSwitches, completedTasks, practiceChoice, totalPrompts, promptsUsed) {
  ...
  analyzeQuery(query) {
    ...
    const helpWords = ['help', 'stuck', 'cant', 'difficult', 'hard', 'struggling'];
  ...
  categorizeQuery(query) {
    ...
    if (q.includes('help') || q.includes('stuck') || q.includes('cant')) return 'help_request';
    if (q.includes('how') && (q.includes('do') || q.includes('work'))) return 'how_to';
    if (q.includes('what') || q.includes('explain')) return 'explanation';
    if (q.includes('strategy') || q.includes('tip') || q.includes('trick')) return 'strategy';
    if (q.includes('rule') || q.includes('requirement')) return 'rules';
    if (q.includes('count') || q.includes('slider') || q.includes('typ')) return 'task_specific';
    if (q.includes('order') || q.includes('sequence') || q.includes('which first')) return 'task_order';
  ...
  calculateAccuracy(taskId, answer, expected) {
    if (taskId.startsWith('g1')) {
      ...
      if (error <= precisionRequired) {
        ...
        return Math.round(90 + 10 * (1 - (error - precisionRequired) / precisionRequired));
      } else {
  ...
  calculateImprovementRate(history) {
    ...
    const firstHalfAvg = history.slice(0, midPoint).reduce((sum, h) => sum + h.accuracy, 0) / midPoint;
    const secondHalfAvg = history.slice(midPoint).reduce((sum, h) => sum + h.accuracy, 0) / (history.length - midPoint);
  ...
  assessDifficulty(taskId, attempts, totalTime) {
    ...
    if (attempts === 1 && totalTime < 10000) return baseDifficulty + '_mastered';
    ...
    if (attempts > 5 && totalTime > 120000) return baseDifficulty + '_very_challenging';
  ...
  analyzeLearningCurve(history) {
    ...
    const recentImprovement = improvements.slice(-3).reduce((a, b) => a + b, 0) / 3;
  ...
  analyzePositionErrors(answer, expected) {
    ...
    if (answer.length === expected.length && answer !== expected) errors.substitution = true;
  ...
  getCompletedTaskCount() {
    const tasks = ['g1t1', 'g1t2', 'g1t3', 'g2t1', 'g2t2', 'g2t3', 'g3t1', 'g3t2', 'g3t3'];
  ...
  getTaskProgress(taskId) {
    ...
    return {
      ...
      averageTime: history.length > 0 ? 
        history.reduce((sum, h) => sum + h.timeTaken, 0) / history.length : 0,
      isImproving: history.length >= 2 ? 
        history[history.length - 1].accuracy > history[history.length - 2].accuracy : false
    };
  },
  ...
  assessQueryRelevance(query, currentTask) {
    ...
    if (game === 'g1' && (q.includes('count') || q.includes('word') || q.includes('letter'))) {
      return 'highly_relevant';
    }
    if (game === 'g2' && (q.includes('slider') || q.includes('slide') || q.includes('target'))) {
      return 'highly_relevant';
    }
    if (game === 'g3' && (q.includes('type') || q.includes('typing') || q.includes('pattern'))) {
      return 'highly_relevant';
    }
  ...
  analyzeSentiment(text) {
    const negative = ['cant', 'wont', 'stuck', 'hard', 'difficult', 'impossible', 'wrong', 'bad'];
    const positive = ['good', 'great', 'easy', 'got', 'understand', 'thanks', 'helpful'];
    const frustrated = ['ugh', 'argh', 'damn', 'frustrat', 'annoying', 'stupid'];
  ...
  // Urgency assessment
  assessUrgency(text) {
    const urgent = ['help', 'please', 'need', 'stuck', 'cant', 'now', '!', '??'];
    const lower = text.toLowerCase();
  ...
  calculateSpecificity(query) {
    ...
    const specificTerms = [
      'count', 'slider', 'typing', 'word', 'letter', 'target', 'pattern',
      'accuracy', 'decimal', 'case', 'symbol', 'highlight', 'enhanced'
    ];
  ...
  identifyStrugglePoints(history, mistakes) {
    ...
    if (!improvement.isImproving && history.length > 3) struggles.push('plateau');
    
    // Specific mistake patterns
    const topMistake = Object.entries(mistakes)
      .sort(([, a], [, b]) => b - a)[0];
  ...
  calculateGameStats() {
    ...
    return {
      overallAccuracy: completedStats.length > 0 ?
        completedStats.reduce((sum, s) => sum + s.bestAccuracy, 0) / completedStats.length : 0,
      bestTask: completedStats.sort((a, b) => b.bestAccuracy - a.bestAccuracy)[0],
      worstTask: completedStats.sort((a, b) => a.bestAccuracy - b.bestAccuracy)[0],
  ...
  getTaskCompletionOrder() {
    const order = JSON.parse(localStorage.getItem('taskCompletionOrder') || '[]');
    return order;
  },
  ...
  getChatInteractionSummary() {
    ...
    return {
      ...
      helpRequests: interactions.filter(i => i.queryType === 'help_request').length
    };
  },
  ...
  getSwitchContext(from, to) {
    ...
    return {
      fromTaskStatus: {
        ...
        lastAccuracy: fromHistory.length > 0 ? 
          fromHistory[fromHistory.length - 1].accuracy : 0
      },
      toTaskStatus: {
        previousAttempts: toHistory.length,
        bestAccuracy: toHistory.length > 0 ?
          Math.max(...toHistory.map(h => h.accuracy)) : 0
      },
